...
from fed_train_glue import drop_peft_model


def train_client(model, dataloader, dropout_rate, args):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)

    # # drop
    print('dropout rate is')
    print(dropout_rate)
    drop_peft_model(model, dropout_rate=dropout_rate, lora_type="A")    #加入drop
    drop_peft_model(model, dropout_rate=dropout_rate, lora_type="B")
